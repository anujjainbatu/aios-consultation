        ...
        >
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className="bg-surface-2 rounded-2xl p-6 flex flex-col gap-4 min-w-[280px] sm:min-w-[340px] max-w-sm snap-start shrink-0"
            >
              {/* Large quote mark */}
              <span
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              onClick={() => {
                ...
                if (!el) return;
                const cardWidth = el.querySelector("div")?.offsetWidth ?? 320;
                el.scrollLeft = i * (cardWidth + 24);
              }}
              className="w-2 h-2 rounded-full bg-surface-2 hover:bg-accent transition-colors duration-200"
